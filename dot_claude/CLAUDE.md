...
- 抽象語で要約したら、その場で具体に置き換える。「何を同じものとみなすか」は「キーの取り方」、「恒久キーとして残る」は「案件に出稿IDが振られている」と書く。読み手が中身を復元できない要約語を残さない
...
- 項目名と説明を並べるときは、名前を主語にした一文にする。「広告アカウントマスター。全量リスト。キーは〜」ではなく「広告アカウントマスターは、〜の全量リスト。キーは〜」
