...
- 日本語で箇条書きの直前に `：` を置かない
- Issue、PR、記事、ドキュメントなど出典のあるものに言及するときは、言及した文の中で Markdown リンクにする。裸の番号・裸の URL を書かない。同一リポジトリでも `#123` ではなく `[#123](https://github.com/org/repo/issues/123)` とフル URL で書く。文末のリンク一覧は本文中リンクの代わりにならない。対象は記事・Issue/PR 本文・ドキュメントで、チャット応答は対象外
