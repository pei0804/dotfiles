---
description: 過去のClaude Codeセッションのトランスクリプト（会話ログ）を検索・閲覧する。「前のセッションで何話してた？」「昨日のセッション見せて」「あのとき何やってたっけ」「過去の会話」「transcript」「セッション履歴」などと聞かれたときに使う。コマンド実行履歴ならaudit-analyzer、Slack発言の振り返りならslack-recallを使う。会話を見るだけでなく中断した作業を再開したい場合はsession-resumeを使う。
---
