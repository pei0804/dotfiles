---
description: 過去のSlack発言からチャンネル別に活動サマリーを生成する。「今週何してた？」「先週の振り返り」「Slackで何話してた？」「活動まとめ」などと聞かれたときに使う。対象はSlackの発言のみ。
allowed-tools:
...
- パス: `~/.agents/skills/slack-recall/config.json`
- 形式: `{"slack_user_id": "UXXXXXXXX"}`

ファイルが存在しない場合:
1. ユーザーに Slack の user_id を聞く
2. `~/.agents/skills/slack-recall/config.json` に書き込む
3. 続行する
