...
```bash
# Markdown → Slack
python ~/.agents/skills/format-converter/scripts/convert.py --to-slack input.md

# Slack → Markdown  
python ~/.agents/skills/format-converter/scripts/convert.py --to-markdown input.txt

# 標準入力から
echo "**bold**" | python ~/.agents/skills/format-converter/scripts/convert.py --to-slack
```
...
**リンク記法は送信経路で使い分ける。** `<url|text>` が有効なのは API 経由で送る場合（Slack API、MCP の送信ツール、下書き作成）だけ。Slack のメッセージ入力欄に貼り付けると、リッチテキストエディタが URL 部分だけを URL として認識し、`|` を `%7C` にエンコードするためリンクが壊れる。貼り付け前提の文面では表示テキストを諦め、生 URL を行末に単独で置く。Slack が自動でリンク化する。表示テキストを付けたい場合は、貼り付けたあとに Slack 上で Cmd+Shift+U で編集する。なお URL の直後に日本語の句読点や全角括弧を置くとリンク検出が巻き込んで無効化するため、URL は行末に置くか半角スペースで区切る。
