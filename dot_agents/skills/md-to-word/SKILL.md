...
pandoc は docx に SVG をそのまま埋め込むこともできるが、Word では SVG が表示されない・崩れることが多い。PNG に変換してから埋め込むと、どの Word でも確実に表示される。
...
4. pandoc で docx を生成する。`--resource-path` で変換済み PNG（`$WORK`）と元の画像（`$DIR`、ラスター画像用）の両方を解決する。`--reference-doc` で罫線付きテーブルのスタイルを当てる。
   ...
   ```bash
   REF="$HOME/.agents/skills/md-to-word/assets/reference.docx"
