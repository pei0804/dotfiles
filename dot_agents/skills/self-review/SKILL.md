...
- `~/.agents/skills/tech-reviewer/SKILL.md` を読み、その「レビューの姿勢」と「レビュー観点（人間にしか判断できないこと）」の PR 向けの問いに従うよう指示（哲学者名・引用を本文に書かない禁止ルールを含む）
- 技術哲学: `~/.agents/references/tech_philosophy.md` を読むよう指示
...
main ブランチにいる場合は必ず worktree を作ってから作業する（`~/.agents/AGENTS.md` の "Worktree First"）。

#### 変更を実装してコミット

- 関連性の高い指摘は 1 コミットにまとめる。無関係な指摘は分ける
- コミットメッセージはリポジトリの規約（Session トレーラー + Co-Authored-By）に従う
