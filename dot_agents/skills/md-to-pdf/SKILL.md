...
2. pandoc で self-contained HTML を生成する。
   ...
   HDR="$HOME/.agents/skills/md-to-pdf/assets/pandoc-header.html"
...
3. Chrome ヘッドレスで PDF 印刷する。ユーザーが Chrome を起動中でも `--headless=new` は別プロセスでレンダリングするため、そのまま使ってよい（`--user-data-dir` を渡すと初回起動のセットアップで固まることがあるので付けない）。
